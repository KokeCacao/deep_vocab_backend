...
def _extract_header_token_value(request_headers):
    ...
    authorization_header = request_headers.get(
        current_app.config["JWT_HEADER_NAME"])
...
def query_jwt_required(fn):
    ...
    def wrapper(*args, **kwargs):
        token = kwargs.get(current_app.config["JWT_TOKEN_ARGUMENT_NAME"])
        try:
            verify_jwt_in_argument(token)
        except Exception as e:
            kwargs[current_app.config["JWT_ERROR"]] = True
...
def mutation_jwt_refresh_token_required(fn):
    ...
    def wrapper(cls, *args, **kwargs):
        ...
        return fn(cls, *args, **kwargs)  # added cls
